        ...
        if (!canSurvive(state, world, pos)) {
            ...
            if (world.getLightLevel(pos.up()) >= 9) {
                for (int i = 0; i < 4; i++) {
                    ...
                    else if (targetState.isOf(ModBlocks.DIRT_SLAB) && canSpread(state, world, blockPos)) {
                        world.setBlockState(blockPos, ModBlocks.PODZOL_SLAB.getDefaultState()
                                .with(TYPE, targetState.get(TYPE)));
                    }
                    // spread to grass slabs preserving type
                    else if (targetState.isOf(ModBlocks.GRASS_SLAB) && canSpread(state, world, blockPos)) {
                        world.setBlockState(blockPos, ModBlocks.PODZOL_SLAB.getDefaultState()
                                .with(TYPE, targetState.get(TYPE)));
                    }
                }
